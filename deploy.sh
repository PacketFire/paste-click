...
kubectl --server="$GKE_APISERVER" \
        ...
        set image deployment/paste-click paste-click=gcr.io/$GCP_PROJECT_ID/paste-click:latest
        
kubectl --server="$GKE_APISERVER" \
        --namespace="$GKE_NAMESPACE" \
        --token="$GKE_BEARER_TOKEN" \
        --insecure-skip-tls-verify=true \
        set image deployment/paste-click paste-click=gcr.io/$GCP_PROJECT_ID/paste-click
